...
#filter(annotated,span_good==F) #Note: dowm from more that were incorrectly copied. The ones left are either as a result of duplicate spacing not copying
...
```{r}
end <- read_csv(here(annotation_path,"post_test_annotated.csv")) |> 
  filter(!is.na(spellchecked)) |> write_csv(here(annotation_path,"post_test_pre_sbert.csv"))
