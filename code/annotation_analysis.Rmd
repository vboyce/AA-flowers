...
show_summary_rstan <- function(model){
  ...
  intervals <- gather_draws(model, `[a-zA-z_(): ]+`, regex=TRUE) |> mean_qi() |> filter(!.variable %in% badlist) |> 
    mutate(lower=round(.lower, digits=3),
           upper=round(.upper, digits=3),
           `Credible Interval`=str_c("[",lower,", ", upper,"]"),
           Term=.variable,
           Estimate=round(.value, digits=3)) %>% 
...
m1 <- stan_lmer(numword ~ trialNum + condition + (1|gameId)+ (1|flower)+(1|player), data=annotated) |> write_rds(here(model_path,"m1.rds"))

```

```{r, eval=F}
 read_rds(here(model_path,"m1.rds")) |> show_summary_rstan() |> write_rds(here(model_path,"word_model.rds"))
```

```{r}
foo <- read_rds(here(model_path,"m1.rds"))
gather_draws(foo, `[a-zA-z_(): ]+`, regex=TRUE) |> select(.variable) |> unique()
```
...
```{r, eval=F}
read_rds(here(model_path,"m4.rds")) |> show_summary_rstan() |> write_rds(here(model_path,"diffgame_sameflower_model.rds"))
