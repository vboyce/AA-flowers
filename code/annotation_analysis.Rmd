...
annotated <-  read_csv(here(annotation_path, "flowers_annotations.csv")) |> 
  ...
  select(gameId, player, flower, trialNum, pResp, normalized_span,color,numchar, numword, condition)
```
...
annotated |> group_by(gameId,condition) |> tally() |> pull(n) |> summary()

message("Utterances / flower")
annotated |> group_by(flower) |> tally() |> pull(n) |> summary()
...
sum_words <- annotated %>% group_by(trialNum, gameId,condition) %>% summarize(sum_char=sum(numchar),
   ...
   # geom_smooth(method="lm", se=T)+
    ...
    all_words <- total+theme(legend.position = "none") 
        ...
        words <- ggplot(annotated,aes(x=trialNum,y=numword, color=condition))+geom_jitter(alpha=.1)+ geom_smooth()+
          labs(y="Words per reference expression", x="Trial Number")+
        theme(legend.position="none", legend.title=element_blank())+
        scale_x_continuous(limits=c(1,24), breaks=c(6,12,18,24))+
        scale_color_brewer(palette="Dark2")
        
        utts <- annotated |> group_by(gameId, trialNum, condition) |> tally() |> 
...
annotated |> group_by(gameId, trialNum, condition) |> select(flower) |> unique() |> tally() |> 
...
```{r}
annotated |> group_by(gameId, trialNum, condition, player) |> select(flower) |> unique() |> tally() |> 
...
d.games <- read_csv(here(paste(read_data_path, "games.csv", sep = "")),
 ...
 sbert_single <- read_csv(here(annotation_path,"pre_sbert.csv"))|> 
    left_join(d.players) |> left_join(d.games) |> mutate(seen="game") |> 
...
sbert_post_test <- read_csv(here(annotation_path,"post_test.csv")) |> left_join(d.games) |> 
  mutate(trialNum=25) |> 
  select(playerId, gameId, condition, text,referent=num, color, trialNum, seen)%>% bind_cols(readRDS(here(annotation_path,'post_sbert_post_test.RData')))  %>% as_tibble()
