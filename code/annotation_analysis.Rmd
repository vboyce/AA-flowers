...
```{r}
sbert_post_test <- read_csv(here(annotation_path,"post_test.csv")) %>% bind_cols(readRDS(here(annotation_path,'post_sbert_post_test.RData')))  %>% as_tibble()

F_mat <- sbert_post_test %>% select(starts_with("...")) %>% as.matrix() #Features
M_mat <- sbert_post_test %>% select(-starts_with("...")) %>% mutate(feature_ind=row_number(), flower=str_c(color,num))
```

## Within flower
```{r}

end_distinctive <- M_mat %>% 
  group_by(flower, color) %>% 
  mutate(combinedId=str_c(gameId,"_",seen)) %>% 
  make_across_df(F_mat, 'cosine') %>% 
  separate(dim1, c("gameId1", "seen1")) |> 
  separate(dim2, c("gameId2", "seen2")) |> 
  mutate(sim = ifelse(is.nan(sim), NA, sim)) %>%
  ungroup()
```
...
end_distinctive |> mutate(same_group=ifelse(gameId1==gameId2, 
                                            "same_group","different_group"),
                          same_color=case_when(
  seen1=="own" & seen2=="own" ~ "own",
  seen2=="other" & seen1=="other" ~ "other",
  T ~"mixed"
))|> ggplot(aes(group=flower, x=color, y=sim, color=str_c(same_group,same_color)))+     
  stat_summary(aes(group=str_c(same_group, same_color)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), geom="pointrange")

            
```
This is really poorly labelled. Needs stats. 
### TODO models
...
end_between <- M_mat %>% 
  group_by(color) %>% 
  mutate(combinedId=str_c(playerId, "_",gameId,"_",seen, "_", flower)) %>% 
  make_across_df(F_mat, 'cosine') %>% 
  separate(dim1, c("player1","gameId1", "seen1" ,"flower1")) |> 
  separate(dim2, c("player2", "gameId2", "seen2", "flower2")) |> 
  mutate(sim = ifelse(is.nan(sim), NA, sim)) %>%
  filter(flower1!=flower2) |> 
  ungroup()
```

```{r}

end_between |> mutate(same_group=case_when(
  player1==player2 ~ "same_person",
  gameId1==gameId2 ~ "same_group",
  T ~ "diferent_group"
), same_color=case_when(
  seen1=="own" & seen2=="own" ~ "own",
  seen2=="other" & seen1=="other" ~ "other",
  T ~"mixed"
))|> ggplot(aes(x=color, y=sim, color=str_c(same_group,same_color)))+     
  stat_summary(aes(group=str_c(same_group, same_color)),fun.data = "mean_cl_boot", position = position_dodge(width=.3), geom="pointrange")+facet_grid(.~same_group)
...
#here we only want sims between an in-game utt and an end description
# note: cor expects features to be in columns so we transpose

pairwiseCosine <- function(matrix1,matrix2) {
    apply(matrix1,2,function(x){
        apply(matrix2,2,lsa::cosine,x)
    })
}
get_sim_matrix = function(df, F_mat, method = 'cosine') {
  df_game <- df |> filter(type=="game")
  df_end <- df |> filter(type=="end")
  feats_game = F_mat[df_game$feature_ind,]
  print(t(feats_game))
  feats_end = F_mat[df_end$feature_ind,]
  print(t(feats_end))
  if(method == 'cor') {
    return(cor(t(feats_game), t(feats_end), method = 'pearson'))
  }  else if (method == 'cosine') {
    return(as.matrix(pairwiseCosine(t(feats_game), t(feats_end))))
  } else {
    stop(paste0('unknown method', method))
  }
}
...
# note this does de-duplicated version
flatten_sim_matrix <- function(cormat, ids_game, ids_end) {
  data.frame(
    dim1 = ids_end[row(cormat)],
    dim2 = ids_game[col(cormat)],
    sim  = as.numeric(cormat)
  ) %>%
    mutate(dim1 = as.character(dim1),
           dim2 = as.character(dim2))
}
...
make_across_df <- function(M_mat, F_mat, method) {
  M_mat %>%
    do(flatten_sim_matrix(get_sim_matrix(., F_mat, method = method),
                          as.character(filter(., type=="game")$combinedId), as.character(filter(., type=="end")$combinedId)))
}

```


** Does not work with grouped data frames, works with non-grouped dfs ** 
```{r}
#test
test_df1 <- tibble(color=c("blue","green"),id=c("game1","game2"), val1=c(.5,.7), val2=c(.9,.4)) |> mutate(type="game")
test_df2 <- tibble(color=c("blue", "green"),id=c("end1", "end2"), val1=c(1,-1), val2=c(-1,-1)) |> mutate(type="end")
test <- test_df1 |> union(test_df2) 
F_mat <- test %>% select(starts_with("val")) %>% as.matrix() #Features
M_mat <- test%>% select(-starts_with("val")) %>% mutate(feature_ind=row_number(), combinedId=id) |> group_by(color)


a <- make_across_df(M_mat, F_mat, method="cosine")

# want there to be one thing that gets grouped (M_mat) and then we separate it by type

sol <- tibble(dim1=c("end1","end2","end1","end2"), 
              dim2=c("game1", "game1", "game2","game2"),
              sim=c(lsa::cosine(c(.5,.9),c(1,-1)),
                    lsa::cosine(c(.5, .9), c(-1,-1)),
                    lsa::cosine(c(.7,.4),c(1,-1)),
                    lsa::cosine(c(.7,.4),c(-1,-1))))
```

# TODO: Correlates of success

# TODO: scale of variation between groups