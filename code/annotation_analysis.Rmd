...
mean_flowers <- per_game |> group_by(condition) |> summarize(mean=mean(n))

per_game |> group_by(condition, n) |> tally() |> rename("Distinct flowers"=n, Trials=nn) |> 
  pivot_wider(names_from=condition, values_from=Trials)

per_person <- annotated |> group_by(gameId, trialNum, condition, player, name) |> select(flower) |> unique() |> tally() |> full_join(d.rounds.final) |> 
  mutate(n=ifelse(is.na(n),0,n))

mean_person <- per_person |> group_by(condition) |> summarize(mean=mean(n))

per_person |> group_by(condition, n) |> tally() |> rename("Distinct flowers"=n, Trials=nn) |> 
  pivot_wider(names_from=condition, values_from=Trials)
```
Per person
...
show_summary_rstan(m_samegame_sameflower) |> write_rds(here(model_path,"samegame_sameflower_model.rds"))
...
show_summary_rstan(m_samegame_diffflower) |> write_rds(here(model_path,"samegame_diffflower_model.rds"))

```

```{r}
between_game_within_flower <- flower_all |> 
  filter(game1!=game2) |> 
  filter(ref1==ref2) |> 
  filter(later-earlier<3) |> 
  mutate(flower=str_c(color,ref1)) |> 
  select(sim, condition, earlier, flower)

m_diffgame_sameflower <-  stan_glm(sim ~ condition*earlier, data=between_game_within_flower) |> write_rds(here(model_path,"m4.rds"))

show_summary_rstan(m_diffgame_sameflower) |> write_rds(here(model_path,"diffgame_sameflower_model.rds"))
...
show_summary_rstan(m_to_end_sameflower) |> write_rds(here(model_path,"to_end_sameflower_model.rds"))
...
m_to_end_diffflower <-  stan_glm(sim ~ condition*earlier*same_player, data=to_end_diff_flower) |> write_rds(here(model_path,"m5.rds"))

show_summary_rstan(m_to_end_diffflower) |> write_rds(here(model_path,"to_end_diffflower_model.rds"))
...
in_end|> 
  ...
  mutate(distance=case_when(
    ...
    T ~ "different game"),
    same_flower=ifelse(ref1==ref2, "Same flower", "diff flower"))|> 
ggplot( aes(x=condition,y=sim,color=condition))+ geom_point(alpha=.01)+    
...
m_within_end <-  stan_glm(sim ~ condition*same_flower*same_game, data=within_end) |> write_rds(here(model_path,"m6.rds"))

show_summary_rstan(m_within_end) |> write_rds(here(model_path,"within_end.rds"))
