...
ggplot(d_flowers_utt_length, aes(x=trialNum, y=total_num_words, color=as.factor(playerId)))+
  scale_color_brewer(palette="Dark2")+
  facet_wrap(~gameId, nrow=1) +
...
ttr <- function(text_){
  text_ <- gsub('[[:punct:] ]+',' ',text_) ####plus remove caps
  ttr_<- type_token_ratio(text__, n.words=10)
  return(ttr_)
}
...
text_block <- d_flowers_text$text_block

#d_flowers_ttr <- text_block %>%
#  mutate(ttr_v=ttr(text_block))
