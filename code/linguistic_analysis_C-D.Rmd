...
ttrMATTR <- function(text_){
  text_ <- tolower(text_)
  total_words = sapply(strsplit(text_, " "), length)
  if (total_words >21){
  text_obj <- tokenize(text_, format="obj", lang="en")
  ttr_<- MATTR(text_obj, window=21)[1]
  return(ttr_)
  }
  else {
  ttr_<- 0
...
d_flowers_text_trial <- d_flowers %>%
    ...
    #select(-c(text, time, playerId)) %>%
    select(gameId, trialNum, condition, text_block) %>%
    distinct() 

d_flowers_ttr_trial <- d_flowers_text_trial %>%
  mutate(ttr_ = ttrMATTR(text_block))
...
ggplot(d_flowers_ttr_trial, 
 ...
 # facet_grid(rows =vars(condition)) +
  xlab("Trial Number") + 
  ylab("Moving Average Type to Token Ratio (MATTR)") + 
  ylim(0,1)+
