...
theme_jmank <- function (base_size = 11, base_family = "Assistant", base_line_size = base_size/22, 
...
d.rounds.final <-  d.rounds %>% left_join(d.games %>% select(gameId, condition)) %>% 
  filter(condition != "coopCartel")%>% anti_join(rounds_exclude)
d.raw_chat.final <- d.raw_chat %>% left_join(d.games %>% select(gameId, condition)) %>% 
  filter(condition != "coopCartel") %>% anti_join(rounds_exclude) %>% write_csv(here("data/processed_data/joined_data/filtered_raw_chat.csv"))
```
...
```{r, fig.height=3, fig.width=5}
d.by_game_metrics %>%
  ggplot(aes(x = gameLength/60, y = gamePoints, color = log(gameNumWords+1))) + 
  geom_point(alpha = .5) +
  ...
  labs(x = "Game length (in Minutes)", 
       ...
       color = "Log Num. Words") + 
  theme_jmank() +  scale_color_steps(low = "#0F85A0", high = "#BD3922")
...
```{r}
ms <- d.by_game_metrics %>%
  mutate(langUse = ifelse(groupNumWords > 0, TRUE, FALSE)) %>% 
  group_by(condition, langUse) %>%
  summarise(mean_nw = mean(groupNumWords),
            ci_nw = 1.96 * sd(groupNumWords) / sqrt(length(gameLength)),
            mean_points = mean(groupPoints),
            ci_points = 1.96 * sd(groupPoints) / sqrt(length(gameLength)))

d.by_game_metrics %>%
  mutate(langUse = ifelse(groupNumWords > 0, TRUE, FALSE)) %>% 
  ggplot(aes(x = groupNumWords, y = groupPoints, color = condition, shape = langUse)) +
  #geom_smooth(aes(group = condition), method = "lm") +
  geom_point(alpha = .5) +
  geom_pointrange(data = ms, aes(x = mean_nw, y = mean_points, 
                                 ymin = mean_points - ci_points,
...
d.round_word_counts %>%left_join(d.games %>% 
  ...
  geom_point(alpha = .2) +
  geom_smooth(method=lm)+
  facet_grid(cols = vars(block_name)) + labs(x = "Trial Number", y = "Log (N. Words + 1)")+ scale_color_bay() + theme_jmank()
ggsave(here(fig_path, "nwords_log_over_time.png"))
...
```{r}
d.by_round_metrics %>% left_join(d.games %>% select(gameId, condition, chatEnabled))
```
