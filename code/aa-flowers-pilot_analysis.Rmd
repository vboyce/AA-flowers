...
data_list <- c("pilots", "collab_lang_20", "compet_lang_20")
...
```{r}
#pull out pilots
game_completion %>% group_by(condition, chat, gameComplete)%>% tally()
```
...
```{r, fig.height=3, fig.width=5}
d.by_game_metrics %>%
  ggplot(aes(x = condition, y = gameLength/60, 
...
```{r, fig.height=3, fig.width=5}
d.by_game_metrics %>%
  ggplot(aes(x = condition, y = gameNumWords, color = condition, shape = languageCondition)) + 
  geom_jitter(width = .1, alpha = .5) +
  stat_summary(fun.data = "mean_cl_boot")+
  #facet_grid(cols = vars(condition))+ 
...
```{r, fig.height=3, fig.width=5}
d.by_game_metrics %>%
  ggplot(aes(x = condition, y = log(gameNumWords+1), color = condition, shape = languageCondition)) + 
  geom_jitter(width = .1, alpha = .5) +
  stat_summary(fun.data = "mean_cl_boot")+
  #facet_grid(cols = vars(condition))+ 
...
d.by_game_metrics %>%
  ...
  geom_errorbarh(data = ms, aes(x = mean_nw, y = mean_points, 
                                 ...
                                 xmax = mean_nw + ci_nw), width = 0) + 
  # ylim(0,1500) + 
  labs(x = "Words Exchanged (Game)", y = "Points Earned (Game)") + 
  scale_color_bay() + theme_jmank() +  
  theme(legend.position = "bottom")
```
