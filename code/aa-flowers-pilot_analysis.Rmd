...
data_list <- c("technical_pilots", "collabmulti_pilot0", 
               "collabmulti_20", "compet_20")
process_all_data(data_list)
join_all_data()
```

```{r read-processed-date, message=FALSE}
d.players <- read_csv(here("data/processed_data/joined_data/players.csv")) %>% distinct()
d.games <- read_csv(here("data/processed_data/joined_data/games.csv")) %>% distinct()
d.rounds <- read_csv(here("data/processed_data/joined_data/rounds.csv")) %>% distinct()
d.raw_chat <- read_csv(here("data/processed_data/joined_data/raw_chat.csv")) %>% distinct()
```
...
```{r}
game_completion <- d.round_results %>% 
  group_by(gameId) %>% 
...
Recast games with a language USE condition
If there were fewer than 3 lines of communication, cast langUse = False
```{r}
d.games <- d.games %>% left_join(d.chat.raw %>% group_by(gameId) %>% summarize(numChat = sum(!is.na(text))) %>% 
                                   mutate(langUse = (numChat >5)) %>% select(-numChat)) %>%
  mutate(langUse = ifelse(is.na(langUse), FALSE, langUse)) %>%
#add a toddle for completed games
...
```{r}
d.round_results.final %>% left_join(d.games %>% select(gameId, condition, chatEnabled, langUse, gameLength, gameComplete)) %>%
  filter(gameComplete) %>%
  mutate(chat = ifelse(chatEnabled, "lang", "nonlang"),
         full_condition = paste0(condition, "-", chat)) %>%
  group_by(gameId, condition, chat, langUse, full_condition, gameLength) %>%
  summarize(totalUtility = sum(playerUtility))%>%
  ggplot(aes(x=gameLength, y = as.numeric(totalUtility), color = full_condition)) + 
  facet_grid(rows = vars(condition), cols = vars(langUse), scales = "free_y") +
  geom_point(alpha = .75) + 
...
d.round_results.final %>% left_join(d.games %>% select(gameId, condition, chatEnabled, langUse,gameLength, gameComplete)) %>%
  filter(gameComplete) %>%
  mutate(chat = ifelse(chatEnabled, "lang", "nonlang"),
         chatUse = ifelse(langUse, "chatUse", "noChat"),
         full_condition = paste0(condition, "-", chatUse)) %>%
  ...
  ggplot(aes(x=as.numeric(gameLength, units="mins"), y = as.numeric(totalUtility), color = full_condition)) + 
...
ggsave(paste0(image_location, "/game_length.png"))
```
...
```{r}
condition = "competCartel"

d.chat %>% 
         left_join(d.games %>% select(gameId, createdAt,condition, langUse, chatEnabled, gameComplete)) %>% 
         filter(langUse, gameComplete) %>%
ggplot(aes(x = repNum, y=total_num_words, color = as.factor(playerId))) + 
  ...
  labs(title="Cooperative + Lang Used", subtitle = "Number of words", y="Number of words", x="Round number", color="playerId")

ggsave(paste0(image_location, "/words_over_rounds.png"))
```
