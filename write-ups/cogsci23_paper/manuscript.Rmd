...
# TODOs
redraft intro & add citations
add figs
consider rerunning wi_game bet flower with a mixed effect (was being slow, so didn't) and other models
...
For the pairwise comparisons of utterances during the game, we take the similarities for utterances that are within 2 trials of each other (ex. an utterance from trial 10 is compared with utterances from trials 8,9,10,11,12). We do this because different flowers were shown to different groups on different trials. This width is a compromise between having enough pairs of utterances and still being able to treat them as coming from one time point in the game. 
...
TODO we compare games within 2 rounds of each other (ex. trial 10 compared to 8, 9, 10, 11, 12)

One of the key claims is that within a partnership, the referring expressions should converge for the same image, but get more different for different images. Convergence would be increasing similiarity (higher cosine similarity in later rounds), divergence would be decreasing similarity in later rounds). 

For the same flower within the same game, similarity increased over rounds (trial: `r stats_text(wi_g_wi_f,6)`). Utterances were more similar if they were produced by the same participant (`r stats_text(wi_g_wi_f,8)`), although this did not interact with trial number (`r stats_text(wi_g_wi_f,7)`). Games in the shared utilities condition may have had higher similarities overall (`r stats_text(wi_g_wi_f,2)`), but this did not interact with trial number (`r stats_text(wi_g_wi_f,3)`) or same participant versus different participant (`r stats_text(wi_g_wi_f,5)`).  

Different flowers within the same game have diverging descriptions that get less similar over time (trial: `r stats_text(wi_g_bet_f,6)`). Terms are overall slightly less similar in Shared Utility groups (`r stats_text(wi_g_bet_f,2)`), but this does not interact with trial (`r stats_text(wi_g_bet_f,3)`). There is not an effect of being said by the same person (`r stats_text(wi_g_bet_f,8)`), or interactions between who said it and trial number (`r stats_text(wi_g_bet_f,7)`) or condition (`r stats_text(wi_g_bet_f,5)`). 

### Between games

Utterances describing the same flower in different games diverged slightly (trial: `r stats_text(bet_g_wi_f,4)`). There was a condition difference with shared utility conditions having more similar descriptions across games (`r stats_text(bet_g_wi_f,2)`); this did not interact with trial (`r stats_text(bet_g_wi_f,3)`). 
...
```{r toend, fig.env = "figure*", fig.pos = "h", fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "Cosine similarities between SBERT embeddings of utterances produced during a game and the post-game descriptions of the flowers.", fig.height=4, fig.width=6, out.width="50%"}
to_end|> 
  mutate(distance=case_when(
    player1==player2 ~ "Same Person",
